...
describe('IntentRouter', () => {
  ...
  describe('classify — explicit intent passthrough', () => {
    it('trusts non-ask intent with confidence 1.0', () => {
      const op = makeOperation({ intent: 'rewrite-selection', prompt: '随便写的' });
      const result = router.classify(op);
  ...
  describe('classify — keyword pattern matching', () => {
    ...
    it.each(cases)('"%s" → %s (%s)', (prompt, expectedIntent) => {
  ...
  describe('classify — unrecognized prompt falls back to ask', () => {
    it('returns ask for generic text', () => {
  ...
  describe('classify — ambiguity detection', () => {
    it('detects ambiguity when two patterns match with close confidence', () => {
  ...
  describe('classify — output target inference', () => {
    it('infers editor-selection-replace for rewrite with editor selection', () => {
    ...
    it('infers editor-insert-after for continue-writing with editor selection', () => {
      const op = makeOperation({
        prompt: '续写',
        surface: 'editor-toolbar',
        context: {
