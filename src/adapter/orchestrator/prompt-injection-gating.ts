...
const BUNDLE_MAP: Record<PromptGateType, InjectionBundle[]> = {
  greeting: ['project_meta', 'capability_hints'],
  smalltalk: ['project_meta', 'capability_hints'],
...
function finalize(decision: PromptGateDecision, inputs: PromptGateInputs): PromptGateDecision {
  ...
  if ((decision.type === 'greeting' || decision.type === 'smalltalk') && bundles.has('active_focus')) {
